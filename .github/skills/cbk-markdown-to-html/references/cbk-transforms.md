...
- Fenced code blocks that are both indented (inside a list item) **and** blockquote-prefixed (e.g. `    > ```text`).

**Detection:**
- **List-item block:** Line matches `^( {4,})(```+|~~~+)(\S*)$` (4+ leading spaces).
- **Blockquote block:** Line matches `^(>+ *)(```+|~~~+)(\S*)$` (blockquote prefix).
- **Blockquote-in-list-item block:** Line matches `^( {4,})(>+ *)(```+|~~~+)(\S*)$` (4+ leading spaces followed by a blockquote prefix). Test this pattern **first** — it overlaps with the list-item case if not handled first.
...
5. For any variant containing a `>` prefix (cases 2 and 3), wrap the placeholder in blank blockquote lines so the parser produces a standalone `<p>TOKEN</p>`:

```
    >            ← blank blockquote line (same indent prefix, > only)
    > CBKCODE0PLACEHOLDER
    >
```

### Step 0e: Fix paragraph + bullet list inside list items

When a plain continuation line (4+ space-indented, not a bullet) is immediately followed by a bullet item line (4+ space-indented `- ` or `* `) with no blank line between them, most markdown parsers merge both lines into a single `<p>` tag instead of producing a `<p>` followed by a `<ul>`.

This case is common when a step explanation ends with a colon (`:`), such as:
...
**Detection:** A 4+-space-indented non-bullet, non-blank, non-blockquote line is immediately followed by a 4+-space-indented line starting with `- ` or `* `.

**Algorithm:**
1. Scan lines.
2. When the current line matches `^ {4,}\S` (indented, non-blank) but does **not** match a bullet (`^ {4,}[-*]\s`) or number (`^ {4,}\d+\.\s`) or blockquote (`^ {4,}>`), and the next line matches `^ {4,}[-*]\s`, insert a blank line between them.
...
## Transform 4a: Markdown Table Styling

Plain markdown tables generated by the `tables` extension have no border or collapse styling.  Add `border="1"` and `style="border-collapse: collapse;"` to every `<table>` that was **not** created by the blockquote transforms.

### Detection

A table was created by a blockquote transform if it already has a `border` attribute (the non-emoji table has `border="1"` and the emoji table has `border="0"`). Skip those; only target tables with no `border` attribute.

### Algorithm

```
For each <table> with no existing border attribute:
  Set border="1"
  If style does not already contain "border-collapse":
    Prepend "border-collapse: collapse;" to the style attribute
```

### Example

Input HTML (from markdown `tables` extension):

```html
<table>
<thead><tr><th>Step</th><th>Description</th></tr></thead>
<tbody><tr><td>Step 1</td><td>Do something</td></tr></tbody>
</table>
```
...
## Transform 4b: Table and List Spacing

(Previously documented as Transform 4. Renumbered to 4b after adding 4a above.)
